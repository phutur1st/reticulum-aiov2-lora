...
Run on the CM5 with rnsd-radio stopped. Verify reception on the RNode
peer or the AiO V2's RTL-SDR.
"""
import os
import sys
import time
...
preamble = int(sys.argv[3]) if len(sys.argv) > 3 else 64
...
err = dev.begin(freq_hz=915_000_000, sf=8, bw_hz=125_000, cr_denom=5,
                ...
                tx_power=power)
print(f"bring-up device_errors={err:#06x} freq=915 preamble={preamble} power={power}")
